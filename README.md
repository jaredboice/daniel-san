...
When you set a syncDate at some point in the future (after the start date of the projections) daniel-san will simply assign that value to the dateStart attribute for that rule and it will begin its cycle at that time. When that same syncDate is eventually found to be less than the start date of the projections (due to manually moving the global start date forward through the normal course of using the program), it will then be used to sync the modulation cycle to that point in the past.  
