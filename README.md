...
Adding a dateStart on the rule object WITH a syncDate will nullify the syncDate if the dateStart property is in the future. If you want to "sync" the starting date for the modulus/cycle phase in the future (after the start date of the projections) then use the dateStart attribute on the rule along with the syncDate. When that same dateStart is eventually found to be less than the start date of the projections (due to manually moving the global start date forward), that dateStart on the rule object will become the new syncDate.
